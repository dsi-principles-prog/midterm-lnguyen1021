...
abrv_count<- function(df, vars){
  df %>% 
    mutate(langcount = count.fields(textConnection({{vars}}), sep = ","))
}

cl_appstore_games %>% 
  abrv_count(languages) %>% 
  select(langcount, everything())
```

# Testing language count feature
```{r}
cl_appstore_games %>% 
...
cl_appstore_games %>% 
  count_by(c("average_user_rating"), by=c(developer)) 
